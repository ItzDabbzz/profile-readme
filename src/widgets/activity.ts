...
const serializers: Record<
  string,
  (item: any, raw: boolean, showLinks: boolean) => { description: string; repo: string }
> = {
  // ... unchanged (intentionally left undocumented per-key for brevity)
};
...
export function activity(events: any, widget: Widget<ActivityConfig>): string {
  ...
  if (config.groupByRepo) {
    ...
    return Array.from(grouped.entries()).map(([repo, items]) => {
      ...
      const body = style === "list" ? renderList(built, showDate)
                 : style === "compact" ? renderCompact(built, showDate)
